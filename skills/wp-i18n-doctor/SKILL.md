...
with the plugin and may have plugin-specific fixes. Fall back to the shared,
canonical copy at the plugin root.

`CLAUDE_PLUGIN_ROOT` is **not** exported into the Bash tool environment, so
resolve the root explicitly:
...
while IFS= read -r c; do
  [ -n "$c" ] && [ -f "$c/scripts/i18n_doctor.py" ] && WPI18N="${c%/}" && break
done <<< "$CANDIDATES"
if [ -z "$WPI18N" ]; then
...
if [ -f "{plugin_path}/.i18n/scripts/i18n_doctor.py" ]; then
  ...
  DOCTOR="$WPI18N/scripts/i18n_doctor.py"
fi
```
